...
async function main() {
  ...
  const instrument2 = await prisma.instrument.create({
    data: {
      ...
      status: 'CONFORME',
      calibrationPeriod: 12,
      nextCalibrationDate: twoMonthsAgo, // En retard
  ...
  const instrument3 = await prisma.instrument.create({
    data: {
      ...
      status: 'CONFORME',
      calibrationPeriod: 6,
      nextCalibrationDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000), // Dans 15 jours
  ...
  const movement1 = await prisma.movement.create({
    data: {
      instrumentId: instrument2.id,
      type: 'ENLEVEMENT',
      movementDate: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000), // Il y a 3 jours
      expectedReturn: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Dans 7 jours
