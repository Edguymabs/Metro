...
export async function seedInterventionConfig() {
  try {
    ...
    if (!existingConfig) {
      ...
      // Récupérer un admin pour créer la config
      const admin = await prisma.user.findFirst({
        where: { role: 'ADMIN' }
      });
      
      if (!admin) {
        console.error('❌ Aucun administrateur trouvé pour créer la configuration par défaut');
        return;
      }
    ...
    console.error('❌ Erreur lors du seed de la configuration d\'intervention:', error);
  }
}
